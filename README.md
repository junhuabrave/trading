...
**Status: v0.1.0, pre-alpha infrastructure.** Schema, codecs, snapshot tooling, sequencer core and
pre-trade risk engine, each with replay-determinism tests. Not a trading system yet: no OMS, router,
venue gateways or market-data handlers. The design documents are in `docs/`.
...
| `tools/sbegen.py` | Generator. Parses the schema, computes and verifies layouts (natural alignment, declared sizes, 16-byte message padding), emits `gen/cpp/trading.hpp`, `gen/py/trading.py`, `gen/layout.md`. |
...
| `core/seq/journal.hpp` | Append-only frame journal per stream. Open validates every frame and truncates a torn tail; sparse index for replay-from-seq; replay through a separate read handle. |
...
| `core/seq/reader.hpp` | Consumer view with gap detection: any missing sequence is refilled from the journal, so consumers see every frame in order exactly once. |
| `core/seq/watermark.hpp` | Tracks the market-data position an engine has consumed and fills an `MdWatermark`. |
...
| `core/risk/risk.hpp` | Pre-trade risk engine. Deterministic state machine over the core stream: limit precedence (account+symbol > account > firm), kill switches (firm/account/strategy, with release), per-account message rate on sequencer time, symbol status/halt/restricted, qty/notional/collar/tick checks, short-sale rules (shortable, ETB-or-locate with locate consumption and return on cancel, Rule 201 price test), open-order cap, gross/net exposure, buying power with margin classes; fills move open notional into position. Wide arithmetic on exposure sums with hard ceilings. |
| `tests/test_risk.cpp` | All 27 reject reasons exercised on purpose with the exact expected reason; locate and buying-power accounting checked to the unit; then 200k random orders and a cold engine that replays the journal and must reproduce every verdict, reason and buying-power figure, plus the final state hash. |
...
## Numbers (single-core sandbox, no pinning, portable BLAKE3, buffered journal)
    ...
    risk evaluate, full check list, accept path:      ~42 ns per order; ~108 ns with the cancel/close path
...
* Open orders, locates and positions live in `std::unordered_map` reserved at start-up.
  The cancel/close path is dominated by its erase; replacing it with an open-addressing
  table keyed by our 64-bit ids is the first tuning step.
...
1. `core/oms`: parent/child order state machine driven by `ChildOrder`, `VenueAck`, `VenueReject`, `ExecReport`, cancel/replace, fee model over `FeeRecord`, `OrderState` emission; the risk engine then consumes real `OrderState`s instead of the test's.
2. `core/router`: venue selection over the fee table and a book view, emitting `RouteDecision` with the rejected alternative recorded.
3. Go codec emitter in `sbegen.py`; Aeron binding on a real host.
