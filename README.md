...
| `core/router/router.hpp` | The smart order router. A strategy library chosen by the account's routing profile: Sweep takes displayed liquidity across venues; DarkFirst rests at the midpoint on an ATS where nothing is displayed and spills to lit when its timer runs out; PassivePost joins the near touch for the rebate and crosses when the wait has cost more than the rebate is worth; MidPeg rests at the midpoint on a lit venue. A plan that is willing to wait says so in `waitNs` and nothing in the router watches a clock — the engine arms a sequenced `Timer` for it, so a replay reads when the wait ended instead of re-deriving it. Client instructions outrank the strategy and are recorded as the reason: `allowDark` is required before anything goes dark, `noInternalize` keeps an order off our own book and off a wholesaler, `postOnly` never takes. Price priority first, because a rebate that buys its way past a better quote is a trade-through, so venues are ordered by price and net cost decides only within a price; where the sweep crosses several prices at once it marks the children ISO, which is lawful only because the better quotes are in the same plan. A better price it cannot reach stops the sweep rather than being stepped over. Then the cost model: price plus the venue's fee or rebate, weighted by the scorecard's fill rate with the shortfall charged at the fallback, plus measured reversion — signed so a buy and a sell are compared by the same code. Every plan records the runner-up and what it would have cost, which is the number a best-execution review asks for. Children carry a release offset so the slowest venue is not the last to see the order. |
...
| `core/seq/sequencer.hpp` | The ordering core: `armTimer`/`fireTimers` turn a strategy's wait into an ordinary sequenced frame, fired earliest first with ties broken by id, because a clock is the one thing a replay cannot reproduce and a message is. stamps `seq`/`seqTs`/`streamId`, journals, publishes. Atomic `submitBatch` (all-or-nothing, `lastInBatch` flag) so an `MdWatermark` and the decision it explains are adjacent. Emits `SessionStart`, periodic durable `Checkpoint`, `Heartbeat`. Injected clock. |
...
| `tests/test_router.cpp` | Regulation NMS as a property over twenty thousand random markets — every venue independently quoting or not, at prices that lock and cross each other, sessions up and down, buys and sells, limits and market orders — checking on every plan that no child takes at a price worse than a quote it did not also take, that ISO is set if and only if the plan crossed more than one price, that the best price taken is the best price anywhere, and that the total never exceeds the parent's leaves. Then named market shapes with an answer that can be worked out by hand: the cost model checked against arithmetic written in the test rather than against its own output; a venue with a rebate big enough to beat a penny, which still loses to the better price; the same price on two venues, where the rebate does decide; a sweep across three prices with ISO set and the parent's leaves never exceeded; a better price with no session, which stops the sweep; a remainder rested where posting pays best; post-only taking nothing; and release offsets that land the children together. |
